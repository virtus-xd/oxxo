...
export default function ProjectDetail({ projectId, onClose }: ProjectDetailProps) {
  ...
  return (
    <div className={`fixed inset-0 bg-zinc-950 z-50 overflow-y-auto transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <button
        onClick={onClose}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16">
              ...
              <div className="relative aspect-video bg-zinc-900 mb-4 overflow-hidden group">
                ...
                {project.gallery.length > 1 && (
                  <>
                    <button
                      onClick={prevImage}
                      className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 backdrop-blur-sm flex items-center justify-center hover:bg-black/70 transition-all opacity-0 group-hover:opacity-100"
                    >
                      <ChevronLeft className="w-6 h-6" />
                    </button>
                    <button
                      onClick={nextImage}
                      className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 backdrop-blur-sm flex items-center justify-center hover:bg-black/70 transition-all opacity-0 group-hover:opacity-100"
                    >
            ...
            <div className="space-y-8">
              <div>
                <h3 className="text-sm tracking-widest text-zinc-500 mb-2">KONUM</h3>
                <p className="text-xl">{project.location}</p>
              </div>

              <div>
                <h3 className="text-sm tracking-widest text-zinc-500 mb-2">ALAN</h3>
                <p className="text-xl">{project.area}</p>
              </div>

              <div>
                <h3 className="text-sm tracking-widest text-zinc-500 mb-2">TAMAMLANMA</h3>
                <p className="text-xl">{project.completion}</p>
              </div>

              <div>
                <h3 className="text-sm tracking-widest text-zinc-500 mb-2">MÜŞTERİ</h3>
                <p className="text-xl">{project.client}</p>
              </div>

              <div>
                <h3 className="text-sm tracking-widest text-zinc-500 mb-2">DURUM</h3>
                <p className="text-xl capitalize">{project.status === 'completed' ? 'Tamamlanmış' : project.status === 'ongoing' ? 'Devam Eden' : 'Konsept'}</p>
              </div>
            </div>
          </div>

          <div className="max-w-4xl mb-20">
            <h2 className="text-3xl font-light mb-6 tracking-wide">Tasarım Hikayesi</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {relatedProjects.map((relatedProject) => (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-light mb-2">{relatedProject.name}</h3>
                    <p className="text-sm text-zinc-400">{relatedProject.location}</p>
                  </button>
                ))}
              </div>
            </div>
